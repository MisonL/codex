...
| `127434cd8b` | TUI/startup | Bound startup terminal probes | TUI startup | Low | P0-direct, implemented 2026-05-06 | `cargo test -p codex-tui --lib terminal_probe`; `cargo test -p codex-tui` compiled and passed through `terminal_probe` tests, then was stopped after hanging on existing `space_hold_timeout_*` tests |
