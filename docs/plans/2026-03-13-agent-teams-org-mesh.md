...
1. **实验性开关是硬边界**
   ...
   - `agent_org` 启用后, 若未显式选择 experience profile, 默认使用正式版 profile `steward`.
...
1. **用户是组织外部委托方, President 不是最终主权方**
   - 用户/人类 sponsor 位于 Agent Org 之外, 是 mission、non-goals、风险偏好与不可逆决策的最终来源.
   - President 是 org 内的最高执行控制器, 负责解释与执行用户意图, 但不得擅自改写 org constitution、突破用户设定的硬边界, 或将自己的推断冒充用户裁决.

1. **约束必须沿层级单向传递, 下层只能细化不能扩权**
   - 约束传递链固定为: `human sponsor -> org constitution -> initiative/team charter -> kickoff -> task`.
   - 下层对象可以细化执行方式与局部验收, 但不得扩大上层 scope、删除 non-goals、放松已 pin 的风险护栏; 若需要改写, 必须显式升级裁决.

1. **同一个控制旋钮只能有一个主控制器**
   - 任何高影响控制旋钮 (org 模式、team 模式、shared boundary、gate policy、staffing budget、release policy) 必须明确 primary owner.
   - Council 的职责是裁决共享边界, 不是与 President/owner/leader 同时写同一个控制旋钮; 否则会产生控制器冲突与振荡.

1. **组织与团队必须有显式运行状态机**
   - Org/Team 不能长期只有 "存在/不存在" 两态; 至少要支持 `forming`, `active`, `constrained`, `incident`, `frozen`, `archived` 这类可治理运行模式.
   - 模式变化必须改变允许动作集合, 并写入可审计事件; 禁止在系统失稳时仍默认接受所有新任务、招募与广播.

1. **人类审批带宽是第一类稀缺资源**
   - 在 Agent World 中, 真正稀缺的不是生成能力, 而是人类/Principals 的决策带宽.
   - review、release、override、重大裁决必须受 `decisionBudget` 约束与观测, 否则会因审批队列饱和引发 anti-windup 问题与组织振荡.

1. **体验版本分流必须与功能开关正交**
   - `/experimental` 负责决定 "Agent Org 功能族是否启用".
   - `/experience` 负责在 Agent Org 已启用时决定 "采用哪条组织体验线".
   - 体验线只能改变组织运行策略、协作形态与控制参数, 不能绕过已 pin 的授权与安全边界.
...
- **人类 sponsor (Human sponsor)**: 位于 org 外部的最终委托方, 负责设定 org constitution 与不可逆边界; 不参与 team mesh.
- **组织宪章 (Org constitution)**: org 级最高约束文档, 定义 mission、non-goals、风险偏好、决策权限与升级口径; President 只能执行与维护引用, 不能自我授权改写.
...
## 0.4.1 Experience 版本线与代号 (Pinned)

为了把更激进的 agent-native 组织形态与正式方案做并行对比实验, Agent Org 在产品层分为两条体验线:

1. 正式版: `Steward`
   - 含义:
     - 强调 stewardship / accountable governance
     - 对应当前文档主线: 宪制优先、层级治理、principals 收敛、低噪声、强审计
   - 定位:
     - 默认正式版本
     - 作为实现与回归的主 baseline
...
1. 分支实验版: `Flux`
   - 含义:
     - 强调 adaptive flow / fluid coordination
     - 用于承载更 agent-native、更高吞吐的组织实验
   - 定位:
     - 仅在 `/experience` 中显式选择后启用
     - 作为与 `Steward` 对照的实验线, 不替代正式基线
...
## 0.4.2 `Steward` 与 `Flux` 的组织学差异 (Pinned)
...
- 治理哲学:
  ...
  - 强调明确层级、明确职责、明确门禁
- 默认特征:
  - principals-only 跨 team 收敛
  - team owner / leader 的管理跨度保守
  - quality/gate 以保守收口为优先
  - 夜间/无人窗口默认只推进低风险事项

`Flux`:

- 治理哲学:
  - 保留硬边界前提下, 更接近 agent-native 的自适应组织
  - 目标是在 7x24 下提高有效吞吐, 而不显著牺牲质量
- 默认实验方向:
  - 受限 liaison 机制: 允许被授权的临时跨 team 协调角色, 但不能演化为自由跨 team mesh
  - capability ledger: 用历史证据而非仅模板静态画像做路由与招募建议
  - manager span guardrail: 对 owner/leader 的直管跨度设硬上限, 超限时建议自动拆 cell/扩 leader
  - unattended lane: 把低风险、证据充分的工作归入可无人值守的夜间推进车道
  - independent quality line: 对高风险任务强化验证线的独立否决权

固定规则:

- `Flux` 不是放松治理, 而是把治理从静态层级部分转移到更动态的控制面机制上.
- `Flux` 引入的任何新自由度都必须伴随:
  - 更强的观测
  - 更明确的回滚
  - 更严格的实验 gate
...
```json
{
  ...
  "modeUpdatedAt": 1739988000,
  "modeReason": null,
  "broadcastPolicy": "leaders_only",
  "teamCharterArtifactId": "art-team-charter-1",
  "workingAgreementArtifactId": "art-team-working-agreement-1",
  "decisionBudget": {
    "maxPendingApprovals": 8,
    "maxHumanAsksPerDay": 5
  },
  "playbooks": {
...
- `operatingMode` 是 team 的运行模式, 至少支持:
  - `forming`: 组建/招募/模板 pin 阶段, 默认禁止正常交付与发布.
  - `active`: 正常交付.
  - `constrained`: 因 review/backlog/预算压力进入受限模式, 默认限制新增招募、高频广播与非关键任务创建.
  - `incident`: 事故处置模式, 默认抬高 incident 相关消息与任务优先级, 并限制普通发布动作.
  - `frozen`: 冻结模式, 仅允许读、求助、incident 收口、offboarding 与显式解冻.
  - `archived`: 归档/解散后的只读模式.
- `teamCharterArtifactId` 与 `workingAgreementArtifactId` 是 team 的组织边界与执行协议入口, 用于 onboarding、kickoff 与 Directory 引用.
- `decisionBudget` 用于约束 team 对 principals/human gate 的带宽消耗, 防止在高吞吐下把审批当作无限资源.
...
```json
{
  ...
  "orgId": "org-123",
  "orgName": "demo-org",
  "humanSponsorRef": "user://current-session",
  "presidentThreadId": "thread-president",
  "createdAt": 1739988000,
  "constitutionArtifactId": "art-org-constitution-1",
  "experienceProfile": "steward",
  "operatingMode": "active",
  "modeUpdatedAt": 1739988000,
  "modeReason": null,
  ...
  "decisionBudget": {
    "maxPendingExecutiveApprovals": 12,
    "maxHumanAsksPerDay": 6
  },
  "playbooks": {
    "harnessPlaybookArtifactId": "art-harness-1",
    "csePlaybookArtifactId": "art-cse-1"
...
- `humanSponsorRef` 用于指向 org 外部委托方, 作为 org constitution 与不可逆边界的来源引用.
- `constitutionArtifactId` 是 org 级最高约束入口, 规定:
  - mission / non-goals
  - 风险偏好与不可突破的边界
  - 何类决策必须升级到 human sponsor / President / Council
- `experienceProfile` 表示当前 org 采用的体验线:
  - `steward` 为正式基线
  - `flux` 为分支实验线
- `operatingMode` 与 `decisionBudget` 的语义与 team 类似, 但作用于 org 级治理.
- Org 与 Team 都必须把 mode 变化视为高信号控制面动作, 不能混在普通 config 更新里被淹没.
...
- Inputs (snake_case):
  ...
  - `operating_mode` (可选)
  - `mode_reason` (可选; 当 `operating_mode` 变化时建议必填)
  - `broadcast_policy` (可选)
  - `team_charter_artifact_id` (可选)
  - `working_agreement_artifact_id` (可选)
  - `decision_budget` (可选):
    - `max_pending_approvals` (可选)
    - `max_human_asks_per_day` (可选)
...
- 当 `operating_mode` 发生变化时:
  - 必须额外追加 `team.mode.changed` 事件并记录 `from -> to` 与 `reason`.
  - `forming/frozen/archived` 等限制模式必须改变后续动作授权与默认策略, 不能只是展示字段.
...
- `human_sponsor_ref` (可选)
- `constitution_artifact_id` (可选)
- `experience_profile` (可选): `steward | flux`
- `operating_mode` (可选)
- `mode_reason` (可选; 当 `operating_mode` 变化时建议必填)
- `decision_budget` (可选):
  - `max_pending_executive_approvals` (可选)
  - `max_human_asks_per_day` (可选)
- `playbooks` (可选):
  - `harness_playbook_artifact_id` (可选)
...
- 当 `constitution_artifact_id`、`human_sponsor_ref` 或 `operating_mode` 变化时, 必须视为高信号治理动作:
  - mode 变化必须额外追加 `org.mode.changed`.
  - constitution 更新必须在事件 payload 中记录 `supersedes` 或等价的前版本引用, 避免双真相.
- 当 `experience_profile` 变化时:
  - 必须追加 `org.experience.changed` 事件
  - 必须记录 `from -> to`、actor、reason 与 expected rollback path
  - 不得隐式修改 constitution 与授权边界; 仅允许切换策略层与实验对象
...
- Org scope:
  - `org.created`
  - `org.config.updated`
  - `org.experience.changed`
  - `org.liaison.proposed`
  - `org.liaison.activated`
  - `org.liaison.revoked`
  - `org.capability_ledger.updated`
  - `org.mode.changed`
  - `org.team.registered`
  ...
  - `team.span.guardrail.triggered`
  - `team.unattended_lane.entered`
  - `team.unattended_lane.exited`
  - `team.quality_hold.placed`
  - `team.quality_hold.released`
...
- `/experience`: 打开当前 org 的体验线选择器与说明面板
...
`/experience` 的最小语义 (Pinned):

- 若 `agent_org` 未启用:
  - 仅展示说明, 不允许切换
- 若 `agent_org` 已启用:
  - 展示当前 `experienceProfile`
  - 可切换:
    - `Steward` (`steward`)
    - `Flux` (`flux`)
  - 切换必须:
    - 要求输入 `reason`
    - 显示差异摘要
    - 写入 `org.experience.changed`
    - 提示是否需要同步 refresh 相关 principals/onboarding packet
...
1. Feature gate (硬前置):
   ...
   - 在 `agent_org` 启用后, 默认 experience profile 为 `steward`.
...
1. 组织宪章 + 运行模式 + decision budget:
   - 为 org/team config 增加 `constitutionArtifactId` / `teamCharterArtifactId` / `workingAgreementArtifactId`.
   - 为 org/team config 增加 `operatingMode` 与 `decisionBudget`.
   - 将 mode 变化做成高信号事件, 并在限制模式下调整工具授权与默认策略.
...
1. UX 后续:
   ...
   - 增加 `/experience` 入口, 用于在 `steward` / `flux` 间切换并显示差异摘要.
...
1. Experience profile 迁移:
   - 未设置 `experienceProfile` 的 org 一律按 `steward` 解释.
   - `flux` 仅能在 `agent_org` 已启用时被选择; 关闭 `agent_org` 时 profile 配置可保留但不生效.
...
1. Human sponsor (组织外部委托方)
   - Org 外: 设定 org constitution、mission、non-goals、风险偏好与不可逆决策边界
   - 不参与 team mesh 与日常调度, 只在需要最终裁决时介入
1. President
   - Org: 创建/更新 org 元数据, 维护 constitution 引用, 注册/解绑 team, 招募 owners, 跨 team 协调
   - Team: 可 override team owner 的治理动作 (用于紧急处置与组织扩张)
   - 固定边界: President 是执行控制器而非最终主权方; 不得擅自改写 Human sponsor 已 pin 的约束
...
#### 14.2.1 Org Constitution (组织宪章与委托边界)

产品目标:

- 让 Agent Org 有一个高于 team charter 与 task kickoff 的稳定约束源, 防止 President/owners 在高速执行中逐步偏离用户真实意图.
- 将 "什么可以被优化" 与 "什么绝不能被牺牲" 显式化, 作为 org 级最高控制参考输入.

最小字段 (建议作为 `constitution` artifact 存在, 并由 `constitutionArtifactId` 引用):

- `mission`: 组织使命
- `nonGoals[]`: 明确不做的事情与边界
- `riskAppetite`: 风险偏好摘要 (例如保守/平衡/激进)
- `irreversibleDecisions[]`: 必须升级到 Human sponsor 的不可逆决策类型
- `delegationModel`: Human sponsor -> President -> Owner -> Leader 的授权边界
- `changeClassesRef`: 采用的变更分级口径 (见 14.22.1)

固定原则 (Pinned):

- org constitution 是上层约束, team charter/initiative charter/task kickoff 只能细化不能放松.
- President 可以维护 constitution 的最新引用与版本, 但不能把自己的推断写成 Human sponsor 已批准的边界.
- 当 mission/non-goals/risk appetite 发生变化时, 必须显式更新 constitution 引用并触发 refresh, 禁止只在聊天里口头通知.

#### 14.2.2 Experience Profiles (`Steward` / `Flux`) (Pinned)

产品目标:

- 在不破坏 Agent Org 硬边界的前提下, 允许组织运行策略分成 "正式主线" 与 "激进实验线", 以便做并行对照实验.

最小字段:

- `experienceProfile`: `steward | flux`
...
- `flux`
  - 分支实验版本
  - 仅在 `/experience` 显式选择后启用
  - 用于验证更高吞吐、更 agent-native 的组织策略

切换规则:

- 选择 `flux` 不得自动放开权限边界, 只能放开已在实验设计里定义的组织策略差异
- 选择 `steward` 必须能立即作为回退安全态
- 切换 profile 必须:
  - 可审计
  - 可回退
  - 不破坏正在运行的 durable truth

`Flux` 当前规划允许实验的差异面:

1. 跨 team 受限 liaison 机制
   - 由 principals 明确任命临时 liaison, 在有限时间窗内承担跨 team 协调
1. capability ledger 驱动的路由与招募建议
   - 更依赖历史证据与表现, 少依赖静态模板
1. manager span guardrail
   - 对 owner/leader 的直管规模设置更硬的上限与自动提示
1. unattended lane
   - 为低风险、证据完备的任务建立夜间自治车道
1. independent quality line
   - 高风险任务中验证线拥有更强独立收口权
...
#### 14.8.1 Artifact Pattern 与上下文/存储边界 (Pinned)

产品原则:

- 上下文窗口不是存储单元. Agent 的推理上下文只承载当前决策所需的最小事实, 不承担长期保存大文本、大日志、大 diff 的职责.
- Artifact 是跨成员、跨阶段、跨节奏的主要协作载体; 聊天消息只负责触发、解释与引用.

固定规则:

- 大对象必须持久化到 artifact 或外部存储引用, 不得直接长期驻留在线程聊天上下文:
  - 大文件全文
  - 完整 diff / patchset
  - 长日志 / trace / profiling 输出
  - 大型目录扫描结果 / 依赖图 / 数据样本集
- agent 间 handoff 默认只传:
  - `artifact_id`
  - `summary`
  - `contentDigest`
  - 最小必要的 `inputRefs`
  禁止把上游完整聊天历史复制给下游成员作为默认输入.
- reviewer / approver / releaser 的默认输入应是 artifact 与 evidence bundle, 不是 producer 与 coordinator 的内部协商 transcript.
- 如果 artifact 引用失效、digest 不匹配或 contentRef 不可解析, 系统必须显式失败并要求补证, 禁止静默回退到“把内容再贴一遍”.
- team mesh 与 principals channel 的 payload 应优先结构化:
  - artifact refs
  - review refs
  - task refs
  - evidence refs
...
#### 14.8.2 组织记忆分层 (Pinned)

为了避免“消息越多、系统越笨”, Agent Org 的记忆必须至少分三层:
...
1. Working memory
   ...
   - 目标: 仅支撑当前决策
1. Episodic memory
   - `harness_run` / `review` / `postmortem` / `digest` / `events.jsonl`
   - 生命周期: 中长期
   - 目标: 记录发生过什么、谁做了什么、结果如何
1. Semantic memory
   - constitution / handbook / template / ADR/RFC / ownership map / directory
   - 生命周期: 长
   - 目标: 固化稳定规则、角色边界与组织知识

固定规则:

- semantic memory 不得直接从聊天中派生为授权事实; 必须经 artifact / durable truth 明确固化.
- episodic memory 的检索结果必须带来源与时间语义, 避免把过期经验当作当前真相.
- working memory 爆量时, 默认先做 artifact 化与 digest 化, 再继续协作, 禁止无限扩上下文窗口.
...
#### 14.10.1 Release Plan Artifact 结构 (Pinned)

`release_plan` 不只是发布说明, 而是 release 控制器的输入对象. 建议最小字段:

- `releaseId`
- `teamId`
- `changeClass` (对齐 14.22.1)
- `target`:
  - 服务/二进制/包/配置对象的引用
- `scopeRef`:
  - 对应 task / initiative / artifact 的引用集合
- `owner`:
  - 本次发布的责任 principal
- `rolloutMode`:
  - `shadow | canary | phased | full`
- `gates`:
  - 需要满足的 review/evidence/release 窗口条件
- `rollbackTriggers[]`:
  - 可机器识别或可人工判定的回滚信号
- `successSignals[]`:
  - 证明发布稳定的信号与观察窗口
- `soakWindow`:
  - 进入下一阶段 rollout 前的最短观察期
- `dependsOn[]`:
  - 必须先完成的 release 或 shared boundary 变更

固定规则:

- `release_plan` 必须引用证据, 不能只写意图:
  - 至少引用 `patch` / `harness_run` / `review_bundle` 中的一类
- `rollbackTriggers` 必须是显式列表, 不能写成模糊描述
- `rolloutMode=full` 只应在低风险变更或已有前置 canary/phased 证据时使用

#### 14.10.2 Release 状态机 (Pinned)

建议状态机:

1. `draft`
   - release plan 已创建, 尚未进入审查
1. `proposed`
   - 已提交 gate, 等待批准
1. `approved`
   - 满足 gate, 可执行 rollout
1. `rolling_out`
   - 正在执行 canary/phased/full
1. `stabilizing`
   - rollout 已完成, 进入观察窗口
1. `completed`
   - 达到 success signals, 正式收口
1. `aborted`
   - 在执行前或执行中被显式中止
1. `rolled_back`
   - 已触发回滚并完成恢复
...
- 同一 `target` 在同一时间窗内最多允许 1 个 `rolling_out|stabilizing` 的 active release, 避免并发 rollout 产生控制器冲突.
- `aborted` 与 `rolled_back` 必须区分:
  - `aborted`: 主要表示未完成切换或主动停止
  - `rolled_back`: 表示已切换且明确执行了回退
- `incident` 或 `frozen` 模式下, 默认禁止新 release 从 `draft/proposed` 进入 `approved`

#### 14.10.3 Rollout / Rollback 控制回路 (CSE)

Primary setpoint:

- 发布在既定风险窗口内完成切换, 且不突破 rollback trigger 与 SLO guardrail.

主要传感器:

- `errorRate`, `latencyP95/P99`, `saturation`, `rollbackTriggerHitCount`
- `releaseStepDurationMs`, `soakWindowElapsedMs`
- `incidentCreatedDuringRollout`

主要执行器:

- `team_release_approve`
- `team_release_abort`
- rollout 步长切换: shadow -> canary -> phased -> full
- 自动降级到 `frozen` 或 `incident` 模式

固定规则:

- 发布必须小步推进; 高风险变更默认优先 `shadow` 或 `canary`
- 若触发 rollback trigger, 默认先回退再讨论, 而不是继续观察碰运气
- `Recovery Target` 必须在 release plan 中可见, 例如:
  - 多久内完成回滚
  - 回滚后需要恢复哪些护栏信号
...
- `severity`:
  - `P0 | P1 | P2 | P3`
- `status`:
  - `detected | triaged | mitigating | monitoring | resolved | postmortem_pending | closed`
- `commanderThreadId`
- `affectedTeams[]`
- `summary`
- `impact`
- `detectedAt`, `resolvedAt`, `closedAt`
- `runbookRef`
- `mitigationRefs[]`
- `rollbackRef` (可选)
- `nextUpdateAt`
...
- `severity` 一旦升级, 必须追加事件并触发对应 cadence/更新频率
- Incident 期间相关 team 可自动切换到 `incident` 模式, 但切换必须显式留痕
- incident 对象是 org 级真相, 不能用多条 team 聊天替代

#### 14.11.2 Incident 状态机与角色分工 (Pinned)
...
1. `resolved`
   - 影响已消除, 等待复盘
1. `postmortem_pending`
   - 已要求复盘与行动项, 尚未关闭
1. `closed`
   - postmortem 与关键行动项已归档或进入 backlog
...
- Affected Team Owners:
  - 对各自 team 的修复、回滚与证据负责
- President:
  - 负责 org 级裁决与外部同步

固定规则:

- 一个 incident 只有一个 active commander
- `mitigating` 阶段禁止多个 teams 各自发布互相冲突的控制动作而无 commander 协调
- 从 `monitoring -> resolved` 必须有 success signals 引用, 不能凭感觉关闭

#### 14.11.3 Postmortem 与行动项闭环 (Pinned)

`postmortem` artifact 最小字段建议:
...
- `postmortem` 不是事故摘要重写, 必须输出可追踪行动项
- 行动项默认回写为 backlog/task, 不能只停留在 artifact 文本
- `incident` 关闭前至少要满足:
  - postmortem 已创建
  - P0/P1 关键行动项已分派或进入显式延期决策
...
#### 14.12.2 使命级联与约束传递 (Pinned)

从第一性原理看, Agent Org 的核心风险不是 "做不出来", 而是 "下层执行越来越快, 但逐层偏离上层意图". 因此必须定义稳定的约束传递链:

```text
Human sponsor
  -> Org Constitution
    -> Initiative Charter / Team Charter
      -> Kickoff Template + Task
        -> Evidence / Review / Release
```

固定规则:

- 下层只能增加细节, 不能放宽上层的 `nonGoals`、风险护栏与升级要求.
- task kickoff 必须引用上层对象:
  - team 内任务至少引用 `teamCharterArtifactId` / `workingAgreementArtifactId`
  - initiative 内任务还必须引用 `initiative charter`
- 若 task/initiative 需要扩大 scope、改变风险接受口径或触碰上层未授权的 shared boundary, 必须显式升级:
  - team 内边界 -> team owner
- 跨 team/shared boundary -> 对应 Council
- constitution 级变化 -> President + Human sponsor
- 审批与发布只是在下游验证 "是否符合上游约束", 不是重新定义上游约束的地方.

#### 14.12.3 Initiative Snapshot 与状态机 (Pinned)
...
- `ownerThreadId`
- `status`:
  - `draft | active | blocked | stabilizing | completed | cancelled`
- `milestones[]`
- `workstreams[]`
- `participatingTeams[]`
- `dependencyMapRef`
- `charterRef`
- `checkpointCadenceRef`
- `decisionQueue[]` (待 President/Council 裁决的 asks 引用)

固定规则:

- `initiative` 的 `status` 只表达跨 team 收敛状态, 不替代 team 内 task 状态
- `blocked` 必须绑定 blocker/decision ask 引用, 不能是无信息阻塞
- `stabilizing` 适用于跨 team cutover 或大规模重构后的观察阶段

#### 14.12.4 Backlog 优先级与就绪语义 (Pinned)

单纯给 task 加 `priority` 不足以形成稳定 backlog, 还需要区分 "重要" 与 "可开始":

- 推荐 planning 元字段:
  - `priority`: `P0 | P1 | P2 | P3`
  - `readiness`: `captured | ready | blocked | scheduled`
  - `initiativeId` (可选)
  - `milestoneId` (可选)
  - `decisionRequired` (可选): 是否需要 principal 裁决才能开始

固定规则:

- `priority` 高不等于可立即开工; `readiness=blocked` 的任务不能仅因优先级高而抢占执行资源
- `decisionRequired=true` 的 task 默认不得直接进入执行, 必须先产出对应 ask/review/charter delta
- backlog 列表应优先展示:
  - 高优先级且 ready 的项
  - 高优先级但 blocked 的项及其阻塞原因
...
#### 14.14.1 多时域治理节拍 (Pinned)

Agent Org 不能只定义 "消息节奏", 还必须定义不同时间尺度上的控制回路, 否则会把战略问题、日常执行问题与事故处置混在一个频道里:

1. Constitutional loop (低频, 高影响)
   - 主体: Human sponsor + President
   - 对象: org constitution、风险偏好、不可逆决策
   - 频率: 按需触发, 默认最低频
1. Portfolio / Initiative loop (中频)
   - 主体: President + owners + initiative owners
   - 对象: initiative charter、资源分配、跨 team 依赖与优先级
1. Team execution loop (高频)
   - 主体: team owner / leaders / members
   - 对象: tasks、reviews、status digests、局部 staffing
1. Incident loop (事件驱动)
   - 主体: Incident Commander + 受影响 principals
   - 对象: 止血、回滚、影响收敛、复盘行动项

固定原则:

- 低频高影响回路不能被高频消息噪声淹没; 高优先级 ask 必须通过 digest/help_request 被显式提炼.
- incident loop 可以临时提升优先级, 但不能长期取代 portfolio 与 constitutional loop.
- 不同时间尺度的对象应有不同默认模板与 cadence, 禁止用一个 status 流承载全部治理问题.

#### 14.14.2 人类决策带宽作为一等预算 (Pinned)

Agent 可以 7x24 推进, 但 human/principals 的批准与裁决能力是稀缺资源. 因此 org/team 都需要显式的 `decisionBudget`:

- Team 级最小字段:
  - `maxPendingApprovals`
  - `maxHumanAsksPerDay`
- Org 级最小字段:
  - `maxPendingExecutiveApprovals`
  - `maxHumanAsksPerDay`

默认执行规则:

- 当审批/ask 队列达到预算上限时:
  - 默认拒绝新的非紧急 review request / executive ask
  - 或要求先汇总为 digest/review_bundle 再进入队列
- decisionBudget 被打满时, 不得继续通过更多 status 消息施压; 必须进入 anti-windup 路径:
  - 合并 asks
  - 延后低优先级任务
  - 显式升级要求调整配额或收缩范围

#### 14.14.3 Status Report Artifact 结构 (Pinned)

建议 `status_report` / `digest` 的最小字段:

- `scope`:
  - `team | initiative | incident | org`
- `since`, `asOf`
- `owner`
- `summary`
- `progressDelta[]`
...
- `status_report` 必须是时间窗口语义, 不是静态快照复制
- `asks[]` 默认最多 3 条; 超额说明摘要粒度不对, 应拆分或转为 backlog/initiative
- `stalenessAt` 到期后若无新报告, UI/Directory 应显式显示陈旧状态, 不能假定“一切正常”
...
#### 14.16.1 Evidence Bundle 与来源证明 (Pinned)

单个 `harness_run` 往往不足以支撑高风险 gate, 因此需要 evidence bundle 口径:

- `evidence_bundle` 建议只聚合引用:
  - `patchRef`
  - `testPlanRef`
  - `harnessRunRefs[]`
  - `reviewRefs[]`
  - `releasePlanRef` (可选)
- 每个 evidence ref 都应保留来源证明:
  - `producerThreadId`
  - `createdAt`
  - `contentDigest`
  - `gateLevel`
...
- bundle 只聚合引用, 不复制全文
- 对高风险变更, `team_task_approve` 与 `team_release_approve` 应优先引用 bundle, 而不是散落的多个 artifact id
- 若 bundle 内不同证据互相矛盾, 必须显式失败并要求补证或升级, 禁止挑选性忽略

#### 14.16.2 Gate 证据分层 (Pinned)

不同变更等级应要求不同证据层:

1. Class A
   - 最少需要 L0/L1 级 `harness_run` + `patch`
1. Class B
   - 需要 `test_plan` + `harness_run` + `review` 或 `review_bundle`
1. Class C
   - 需要 release plan + 回滚触发 + soak window + 相关 evidence bundle
1. Class D
   - 在对应等级基础上, 额外需要 Security/Privacy 决策记录

固定规则:

- 证据缺口必须显式显示为 `uncovered[]`, 不能用“待后续补”模糊处理
- L2 风险不能被 L0/L1 绿灯掩盖

#### 14.16.3 确定性验证栈 (Pinned)

第一性原理:

- LLM 负责提出实现假设; 工程正确性必须由确定性工具裁决.
- 在大型代码库中, `grep` / 向量检索 / 自然语言自审都不能替代符号级、类型级与编译级验证.
...
1. Symbol / LSP 层
   ...
   - 类型悬停 / 签名检查
   - diagnostics
1. Compile / Typecheck 层
   - 编译器 / 类型检查器 / schema 校验器
1. Static Analysis 层
   - lint / clippy / eslint / dead code / complexity / unsafe pattern 审查
1. Test 层
   - 组件测试
   - 集成测试
   - 需要时的调试器 / profiler / trace 观测

固定规则:

- 任何代码类 `patch` 在进入 review gate 前, 至少必须附带一组确定性验证结果 artifact, 不能只附自然语言“已检查”.
- 若 LSP / 编译器 / 静态分析与模型判断冲突, 以确定性工具输出为准.
- 结构化诊断必须 artifact 化:
  - `diagnosticRef`
  - `toolName`
  - `severity`
  - `symbolRef` (可选)
  - `contentDigest`
- search / grep 只用于发现候选范围, 不得作为“已验证跨引用正确性”的证据.
- 对 shared boundary 变更, 必须至少补一类符号级验证与一类编译/类型级验证; 否则默认视为证据不足.

#### 14.16.4 顶尖代码审查矩阵 (Pinned)

Agent Org 的 review 不应只判断“能不能运行”, 而应覆盖顶尖软件工程组织的最小审查维度:
...
1. Concurrency & Safety
   - 是否引入竞态、死锁、重复执行、资源泄漏或权限绕行
1. Test & Evidence Adequacy
   - 是否提供了与变更等级匹配的测试与证据
1. Operability & Recovery
   - 是否给出超时、回滚、降级、日志与观测口径

固定规则:

- `approve|reject` 不应只是布尔值; reviewer 至少应附一个结构化原因类别:
  - `design`
  - `complexity`
  - `safety`
  - `test_gap`
  - `operability`
  - `policy`
- 对 Class B/C/D 变更, review 结论必须显式说明:
  - 为什么可以合并
  - 还剩什么未覆盖
  - 谁接受了残余风险
- 任何“安慰剂测试”或无法证明失败面被覆盖的测试, 默认视为 `test_gap`.
...
#### 14.17.1 Help Request 生命周期 (Pinned)

建议 `help_request` 的状态机:

1. `open`
   - 已创建, 尚未被 principal 接手
1. `acked`
   - 已被 leader/owner/president 收悉
1. `in_progress`
   - 正在处理或已要求补证
1. `resolved`
   - 决策已给出, 或 blocker 已解除
...
- 同一 blocker 在冷却窗口内默认只允许 1 个 active `help_request`
- `expired` 不等于已解决; 过期项必须显式重新打开或重新提交
- `acked` 只表示已接手, 不等于批准或提供了决策

#### 14.17.2 升级梯度与响应预算 (Pinned)

建议升级梯度:

1. member -> team leader
1. leader -> team owner
1. owner/leader -> org principal / council
1. President -> Human sponsor
...
- 升级必须逐级发生, 除非 blocker 天然属于更高层边界
- 若 `decisionSlaMs` 连续失守, 默认应触发:
  - `constrained` 模式
  - digest 合并
  - 或范围收缩决策
...
#### 14.18.13 Template Change Control Contract (仅组织形式)

模板不是普通文案, 而是组织控制参数的载体. 因此模板变更必须有独立闭环, 否则会在高吞吐下把局部改动放大成全 org 漂移.
...
1. `semantic`
   - 改变字段解释、模板默认值或推荐工作方式
   - 默认需要 owner + 小范围 pilot 证据
1. `control_plane_affecting`
   - 改变 DoD、gate、WIP、quiet hours、help escalation、cadence 等会影响实际行为的模板
   - 默认需要对应 Council 或 President 裁决
1. `security_critical`
   - 涉及权限、敏感数据、发布窗口或安全门禁
   - 默认进入 Security & Privacy Council

固定要求:

- 每次模板变更都必须声明:
  - setpoint (希望改善什么)
  - required evidence (至少引用 `harness_run` / `test_plan` / `review_bundle` 中的一类)
  - known delays / rollout window
  - rollback trigger 与 cooldown
- propose -> pilot -> adopt -> pin -> deprecate 不是口号, 而是控制闭环:
  - propose: 形成可引用条目并写明收益/新成本/失效模式
  - pilot: 在有限 team/initiative 范围验证
  - adopt: 由唯一主控制器裁决
  - pin: 更新引用版本
  - deprecate: 给出退出窗口与回退路径
- 当出现 `templateVersionDivergence`、`policyViolationRate`、`reviewBacklogDepth` 等传感器恶化时, 默认停止继续推广并回滚到上一 pin 版本.
...
#### 14.19.2 单一主控制器与控制器冲突治理 (Pinned)

为了避免 President、owner、leader、Council 同时调整同一个控制旋钮, Agent Org 必须执行 "one knob, one primary controller":

- org constitution / human sponsor 边界:
  - Primary controller: Human sponsor
  - 执行维护者: President
- org 模式、Directory、org 级配额与 teams 注册:
  - Primary controller: President
- team charter、working agreement、team 模式、team staffing budget:
  - Primary controller: Team owner
- tasks、kickoff、日常分派与 team 内协作节奏:
  - Primary controller: Team leader
- shared boundary / shared contract:
  - Primary controller: Architecture Council
- gate policy / review policy:
  - Primary controller: Quality & Gate Council
- incident 优先级与恢复节奏:
  - Primary controller: Incident Commander (仅 incident 窗口内)

冲突处理规则:

- Secondary actor 只能提案或升级, 不能直接重写不属于自己的控制旋钮.
- 若两个控制器都对同一对象有影响, 必须明确主次顺序与生效窗口; 否则默认视为设计缺陷.
- override 必须保留原因、时间窗与恢复条件; override 不是常态写路径.

最小 Decision Knob Register:

- `org.constitution`
  - controller: Human sponsor
  - requiredEvidence: constitution artifact + supersedes/ref 记录
- `org.operating_mode`
  - controller: President
  - requiredEvidence: 当前背压/incident/预算信号
- `team.working_agreement`
  - controller: Team owner
  - requiredEvidence: 团队 retrospective 或模板变更证据
- `shared_boundary`
  - controller: Architecture Council
  - requiredEvidence: ADR/RFC + rollout/rollback 条件
- `review_policy`
  - controller: Quality & Gate Council
  - requiredEvidence: review backlog / false convergence / gate failure 信号
- `release_window`
  - controller: Release Council
  - requiredEvidence: release plan + rollback trigger
...
#### 14.20.1 组织运行状态机 (Pinned)

组织扩张前必须先定义可回到安全状态的模式切换, 否则 org 会在高吞吐下持续接单直到失稳.

Org/Team 的推荐运行模式:

1. `forming`
   - 用于组建、模板 pin、招募与 onboarding.
   - 默认禁止正常 release 与大规模任务扩张.
1. `active`
   - 正常交付模式.
1. `constrained`
   - 因 review/backlog/预算/人类带宽不足进入受限模式.
   - 默认限制新增招募、非紧急 review request 与高频广播.
1. `incident`
   - 事故或强制恢复模式.
   - 默认提升 incident/status/help_request 优先级, 并限制普通变更与发布.
1. `frozen`
   - 冻结模式.
   - 仅允许读取、incident 收口、offboarding、help escalation 与显式解冻动作.
1. `archived`
   - 归档/解散后的只读模式.

固定规则:

- mode 变化必须由 owner 或 President 触发并写入高信号事件, 不能只改普通描述字段.
- 任何自动化都不得绕过 `frozen` 模式继续创建 task/recruit/release.
- `incident -> active` 的恢复必须伴随显式复盘或恢复依据, 避免刚退出事故又立刻重入.
...
#### 14.21.1 治理产物真相层级 (Truth Hierarchy)

为避免 Template、Handbook、Directory、Charter、ADR 同时承载同一规则而形成双真相, 组织治理产物必须分层:

1. Durable truth
   - `config.json` / `tasks/*.json` / `events.jsonl`
   - 用途: 授权、状态机、可回放事件
   - 规则: 这是唯一授权真相
1. Governance artifacts
   - constitution, charter, working agreement, template, handbook entry, ADR/RFC, runbook
   - 用途: 解释组织形式与执行口径
   - 规则: 通过 `*ArtifactId` / `*Ref` 被 durable truth 引用, 自身不直接授予权限
1. Derived views
   - Directory、索引、query 结果、dashboard rollups
   - 用途: 低噪声展示与快速定位
   - 规则: 必须可从 durable truth + governance artifacts 重建, 不能成为单点真相
...
- 所有治理类引用至少应具备:
  ...
  - `storageRef`
  - `deprecatedAt` (可选)
- 引用解析失败必须显式报错并携带可追溯元信息; 禁止静默回退到聊天复制粘贴.
- 当 Directory 与 durable truth 冲突时, 以 durable truth 为准, 并产生一次显式修复事件或 digest.

#### 14.21.2 ADR / RFC 作为可执行治理对象 (Pinned)

为防止组织约束只停留在自然语言口号层, 共享边界与高风险变更应通过 ADR / RFC 固化为可执行治理对象:

- RFC
  - 用于方案探索、替代方案比较、桥接期设计
- ADR
  - 用于已经裁决的长期约束、战略不变量与必须持续满足的架构决定
...
- Class B/C/D 变更若触碰 shared boundary、release path、security/privacy 口径, 默认必须绑定 ADR 或至少一个待转正的 RFC.
- ADR / RFC 不直接授予权限, 但可作为以下 gate 的必需引用:
  - `team_review_request`
  - `team_release_plan_create`
  - `team_task_approve`
- `Steward` 下:
  - 共享边界变更默认要求 ADR
- `Flux` 下:
  - 可先用 RFC 进入实验或桥接期, 但在合入 shared boundary 之前必须提升为 ADR 或显式被拒绝
- `fitnessFunctions[]` 必须能映射到可执行检查或可观测信号, 禁止写成无法验证的口号.
...
1. Integrate (集成与变更队列)
   - 主责: Build Leader + Reviewer + CI/Gate 自动化
   - 输出: branch/worktree refs、PR/change set、CI 结果、review 状态
...
#### 14.22.1 变更分级与默认治理路径 (Pinned)

为了让 Agent 的高吞吐不把所有任务都推入同一套 gate, 组织必须先对变更做分级, 再决定治理强度:

1. Class A: Team-local change
   - 不触碰 shared boundary, 不涉及 release/security 高风险
   - 默认由 team owner/leader 在 team 内闭环
1. Class B: Shared-boundary change
   - 涉及跨 team 接口、共享 schema、共用基础设施
   - 默认升级到 Architecture Council 或 initiative 协调层
1. Class C: Release-critical / irreversible change
   - 涉及 cutover、回滚窗口、数据迁移、发布窗口
   - 默认需要 Release Council 或 President 裁决
1. Class D: Security / Privacy / Compliance change
   - 涉及凭证、权限、敏感数据、供应链安全
   - 默认进入 Security & Privacy Council
1. Incident lane
   - 用于正在发生的故障与止血动作
   - 默认由 Incident Council/Commander 主导, 并临时覆盖普通 cadence

固定原则:

- 变更分级必须在 kickoff 或 review_request 时被显式声明, 不能等到 gate 才补判.
- 不允许把 Class B/C/D 伪装成 Class A 以绕过治理.
- 变更分级一旦升级, 只有明确裁决后才能降级回普通路径.

#### 14.22.2 Git-native 并行开发与集成隔离 (Pinned)

多 agent 并发开发默认必须建立在 Git 原生隔离之上, 而不是共享工作目录上的并发写入:

- 每个 active assignee / member 默认使用独立 `worktree` 或等价 branch 隔离.
- 同一 task 的多成员可并行开发, 但其物理写入面必须隔离.
- shared workspace 上的直接并发写入默认禁止; 只允许串行集成.

集成规则:

1. 先合入一个 change set 到集成基线
1. 其余 change sets 必须基于新基线 rebase / replay
1. 再进入下一轮 review 与 gate

固定规则:

- 允许自动解决的冲突仅限机械或显然结构化的冲突; 业务语义冲突与架构分歧必须升级到 principal / human.
- `ownership map` / `CODEOWNERS` 类映射只用于路由建议与 reviewer 建议, 不自动扩大任何 agent 的授权面.
- `worktree` / branch / commit / PR ref 应进入 `patch` 或 `harness_run` 证据链, 保证可回放.

#### 14.22.3 PR 作为自然背压阀 (Pinned)

为解决 Agent-time 与 Human-time 的双节奏冲突, 对代码变更默认采用 PR-first 集成模型:

- 非 trivial 代码变更完成后, 默认先形成 `patch` artifact + PR/change set 引用, 而不是直接进入 trunk.
- PR 队列承担三种职责:
  - 人类审查缓冲池
  - review / CI 状态展示面
  - 合并与回滚的自然边界
...
- `unattended lane` 可以创建 PR / change set, 但不得在高风险范围内绕过 review 直接合主线.
- review backlog、CI backlog、merge queue depth 都属于一等背压信号, 必须可观测并参与 `decisionBudget`.
- 当 PR 队列拥塞时, 系统优先做 digest / review_bundle / 范围收缩, 不允许继续无限堆积新的待审产物.

#### 14.22.4 AI-DLC 阶段产物合同 (Pinned)

为了让 Agent Org 不只是“会写代码”, 而是覆盖完整研发生命周期, 各阶段应有默认产物合同:

1. Align
   - `prd` / `acceptance` / 非目标约束
1. Design & Plan
   - `rfc` / `adr` / `plan` / `test_plan`
1. Build
   - `patch` / `harness_run`
1. Verify
   - `evidence_bundle` / `review_bundle`
1. Integrate
   - PR/change set ref / CI result / reviewer decision
1. Release
   - `release_plan` / rollout digest / rollback trigger
1. Run & Learn
   - `postmortem` / `runbook` delta / template delta

固定规则:

- 阶段跳跃必须显式说明缺失了哪些产物与护栏.
- 任何需要 principal / human 做最终裁决的阶段, 默认必须有可引用 artifact, 禁止只靠即时聊天口头描述.

#### 14.22.5 顶尖代码审查标准 (Pinned)

Agent Org 的默认 review 标准应贴近顶尖软件公司, 最低覆盖:
...
- 本节不产生第二套 review truth; 具体审查维度与证据口径以 14.16.4 / 14.16.2 为准.
- reviewer 不仅审“对不对”, 还审“是否让代码库更健康”.
- 对共享边界与高风险变更, reviewer 默认有权基于设计、复杂度或可恢复性拒绝合入, 即使代码能够编译通过.
- “通过”必须绑定可回放证据, “拒绝”必须绑定结构化理由类别, 便于后续训练模板与治理策略.
...
### 14.25 Experience Profiles 与实验包 (仅组织形式)

产品目标:

- `/experience` 不是简单的 UI 选项, 而是一个受控实验面.
- 它的职责是把 Agent Org 的组织策略打包成可比较、可回退的 profile, 让正式版与实验版可以在同一功能族下做 A/B 式演进.

固定原则 (Pinned):

- profile 切换不改变硬授权边界, 只改变组织策略与实验对象.
- profile 必须 bundle 化:
  - 不是零散开关拼装
  - 而是一组经过约束的 policy pack
- 同一 org 同一时刻只能有一个 active `experienceProfile`.
...
- `routingMode`:
  - `template_first`
- `staffingMode`:
  - `owner_leader_planned`
- `decisionRecordMode`:
  - `adr_required_for_class_b_plus`
- `integrationMode`:
  - `pr_first_with_required_reviews`
- `nightShiftMode`:
  - `restricted`
- `qualityGateMode`:
  - `owner_or_quality_council`
- `managerSpanMode`:
  - `advisory`
...
- `crossTeamMode`:
  - `principals_plus_liaison`
- `routingMode`:
  - `capability_ledger_first`
- `staffingMode`:
  - `adaptive_staffing`
- `decisionRecordMode`:
  - `rfc_then_adr_on_boundary_merge`
- `integrationMode`:
  - `pr_first_with_fast_lane`
- `nightShiftMode`:
  - `unattended_lane_enabled`
- `qualityGateMode`:
  - `independent_quality_line`
- `managerSpanMode`:
  - `hard_guardrail`
...
- Primary setpoint:
  - 在不突破质量与边界护栏的前提下, 比较 `Steward` 与 `Flux` 的吞吐、返工与裁决时滞表现
- 必备观测:
  - `throughputPerDay`
  - `reworkRate`
  - `escapedRiskCount`
  ...
  - `suppressionRate`
- 回滚触发:
  - `escapedRiskCount` 持续恶化
  - `decisionBudget` 被系统性打满
  - `crossTeam` 越界或不可回放状态出现

最小实现顺序建议:

1. 先实现 `Steward`
2. 再把 `Flux` 作为受限 profile 叠加在同一控制面对象上
3. 禁止为 `Flux` 单独复制一套控制面, 避免双真相与维护分叉

#### 14.25.1 `Flux` 机制一: 受限 Liaison 机制 (Pinned)

设计目标:

- 在不开放 member 级跨 team 自由直连的前提下, 让某些高频跨 team 协调从 principals 的纯串行转发中解压出来.
...
- `liaison_lease`
  ...
  - `allowedActions[]`
    - 最小只允许:
      - `org_principal_message_proxy`
      - `org_artifact_republish`
      - `status_digest_forward`
  - `scopeRefs[]`
  - `leaseUntil`
...
- liaison 不是 target team 成员, 不获得 target team 内部成员级工具权限
- liaison 只能在 org scope 工作, 不能绕过 principals-only 的安全边界
- `leaseUntil` 到期后默认失效, 必须显式续约
- liaison 的所有动作都必须带 `liaisonId` 与 sponsor 引用, 便于回放责任链

#### 14.25.2 `Flux` 机制二: Capability Ledger (Pinned)

设计目标:

- 用历史证据表达 "谁更擅长什么、谁更稳定", 让路由与招募建议逐步从模板静态画像迁移到证据驱动.

最小对象:

- `capability_ledger_entry`
  - `subjectType`: `thread | team`
  - `subjectId`
  - `domains[]`
  - `platforms[]`
  - `artifactKinds[]`
...
- capability ledger 是派生视图, 不得作为授权依据
- 任何 ledger 结论都必须可回溯到 evidence refs
- 过期或证据不足的条目只能降权, 不能假装仍然可信
- `Flux` 中的路由/招募建议可以优先引用 ledger, 但不得完全忽略 team owner 的最终裁决

#### 14.25.3 `Flux` 机制三: Manager Span Guardrail (Pinned)

设计目标:

- 人类组织中管理幅度过大会导致决策拥堵; 在 agent 世界里, 该问题会被 7x24 吞吐放大.
...
- `Flux`:
  - 进入 hard guardrail:
    - 超限时默认拒绝继续增加直管成员或新增高优先级 workstream
    - 优先建议:
      - 委派新 leader
      - 拆 cell
      - 下放低风险审批
...
- 任何硬限制都必须能被 President/owner 显式 override, 但必须留痕
- span 超限时, 系统优先保护决策质量, 而不是继续追求并发膨胀

#### 14.25.4 `Flux` 机制四: Unattended Lane (Pinned)

设计目标:

- 为低风险、证据充分的工作建立夜间与无人窗口自治车道, 使系统能 7x24 推进而不把所有事项都堵在人类在线窗口.

最小准入条件:

- `changeClass = Class A`
- 不触碰 shared boundary / release-critical / security-sensitive 范围
- `test_plan` 与自动化 evidence 已存在
- `uncovered[]` 为空或被预先接受到允许范围
- team working agreement 明确允许进入 unattended lane
...
- 一旦出现:
  ...
  - 连续失败
  - 需要 principal 决策
  则必须退出 unattended lane, 进入 `handoff_pending`

#### 14.25.5 `Flux` 机制五: Independent Quality Line (Pinned)

设计目标:

- 防止高吞吐组织里 Build 线与 Gate 线合流, 导致“自己产出、自己放行”的假收敛.

最小对象:

- `quality_hold`
  - `holdId`
  - `scope`: `task | release | initiative`
  - `targetId`
  - `placedByThreadId`
  - `reason`
  - `evidenceRefs[]`
...
- 对 Class C / Class D / Incident lane 相关对象, `Flux` 默认允许独立质量线放置 `quality_hold`
- `quality_hold=active` 时:
  - `team_task_approve`
  - `team_release_approve`
  默认必须失败
- 只能由:
  - 原放置者释放
  - 或 President 显式 override
- override 必须要求理由、时间窗与风险接受记录

#### 14.25.6 `/experience` 切换迁移语义 (Pinned)

切换 `experienceProfile` 不是普通配置修改, 需要迁移语义:

- 新对象:
  - `experience_switch_record`
    - `switchId`
    - `orgId`
    - `fromProfile`
    - `toProfile`
    - `requestedBy`
    - `reason`
    - `status`: `requested | scheduled | active | rolled_back | cancelled`
    - `effectiveAt`
...
- 对进行中对象:
  ...
  - active `release`
  - active `quality_hold`
  - active `liaison_lease`
  默认保持创建时 profile 语义, 直到显式 rebind 或对象关闭

切换限制:

- 存在 active `incident` 或 `rolling_out release` 时, 默认只允许 `scheduled`, 不允许立即激活, 除非 President override
- 从 `flux -> steward` 必须被视为标准回退路径, 优先级高于继续实验

#### 14.25.7 实验评估合同与停损规则 (Pinned)

实验不是“试试看”, 必须有比较合同:

- 对比窗口:
  - 最小 observation window 建议按 team/initiative 粒度定义, 禁止只看单日尖峰
- 关键指标:
  - `throughputPerDay`
  - `leadTime`
  - `reworkRate`
  ...
  - `timeToDecisionMs`
  - `evidenceCompletenessRate`
  - `managerSpanViolations`
  - `nightShiftYield`
- 停损规则:
  - 任一质量护栏持续恶化
  - 出现不可回放状态
  - 跨 team 越界或实验绕权
  - `decisionBudget` 被持续打满且无改善

固定规则:

- `Flux` 的收益必须用证据证明, 不能只凭主观“感觉更快”
- 若吞吐提升但返工率、escaped risk 或决策时滞显著恶化, 默认判定实验失败

#### 14.25.8 事件、接口与作用域边界 (Pinned)

最小事件面:

- `org.experience.changed`
- `org.liaison.proposed` / `org.liaison.activated` / `org.liaison.revoked`
- `org.capability_ledger.updated`
- `team.span.guardrail.triggered`
- `team.unattended_lane.entered` / `team.unattended_lane.exited`
- `team.quality_hold.placed` / `team.quality_hold.released`

最小资源/API 面 (仅规划):

- `org_experience_read`
- `org_experience_switch`
- `org_liaison_upsert` / `org_liaison_revoke` / `org_liaison_list`
- `org_capability_ledger_read`
- `team_quality_hold_place` / `team_quality_hold_release`

作用域边界:

- 第一阶段只支持 org 级单一 active profile
- 不支持 team 级行为性 override, 避免同一 org 内出现多套实时控制语义
- 若需要小范围试点:
  - 允许 `shadow profile` 做观测与推荐
  - 但不允许真正改变行为, 直到 org 级切换被批准
...
- org/team 级:
  ...
  - `maxPendingApprovals`, `maxPendingExecutiveApprovals`, `maxHumanAsksPerDay` (限制 human/principal 决策带宽被打满)
- 运行时:
  ...
  - 当 decisionBudget 被打满时, 必须显式进入 `constrained` 或等价限制模式, 而不是继续接受更多 asks/reviews 导致 anti-windup.

#### 15.2.2 配额作用域与继承规则 (Pinned)

为避免 org/team/task 各自定义配额但互相冲突, 需要明确作用域:

1. Org 级配额
   - 全局上限, 例如 `maxTeams`, `maxAgents`, `maxPendingExecutiveApprovals`
1. Team 级配额
   - team 的局部预算, 不能突破 org 上限
1. Initiative 级配额
   - 用于临时大项目, 只能在所属 teams 的配额窗口内调度
1. Task 级预算
   - 例如 `delayBudget`, `evidenceBudget`, `askBudget`

固定规则:

- 子级预算只能收紧, 不能突破父级
- 若父级预算耗尽, 子级即使有剩余额度也必须受限
- 临时 override 必须带过期时间与原因

#### 15.2.3 统一失败语义 (Pinned)

建议为预算与配额相关动作统一错误码族:

- `quota_exceeded`
- `decision_budget_exhausted`
- `review_backlog_saturated`
- `depth_limit_reached`
- `rollup_required`
- `mode_restricted`
...
#### 15.2.1 Quota / Suppression Control Loop

配额与抑制不能只是一组静态常数, 必须有控制闭环:

- setpoint:
  - `timeToDecisionMs`、`oldestReviewAge`、`helpRequestSlaMs` 保持在可接受区间
- guardrails:
  - `suppressionRate` 不能高到让关键 ask 触达失败
  - `reviewBacklogDepth` 不能高到让 gate 失去时效
- sensors:
  - `suppressionRate`, `rollupRate`, `reviewBacklogDepth`, `oldestReviewAge`, `timeToDecisionMs`, `contextPasteRate`
- actuators:
  - 配额调整、quiet hours、rollup 冷却窗口、WIP 限制、digest 强制化
- owner:
  - 默认由 Quality & Gate Council 负责 team/org 级配额治理; org 级 override 由 President 持有

固定规则:

- 调参必须可审计并具有冷却窗口, 防止频繁放开/收紧导致振荡.
- 当 `suppressionRate` 过高且 `timeToDecisionMs` 恶化时, 必须视为过度抑制, 而不是继续加码静默策略.
...
#### 15.4.1 治理策略可执行性矩阵 (Pinned)

- `broadcastPolicy`
  - 来源: team config
  - 执行层: code authz + tool validator
  - 证据: `team.config.updated`, `team.message.appended`, `delivery.suppressedReason`
  - 失败语义: 显式拒绝或抑制并给出原因
- `quietHours`
  - 来源: team config / working agreement
  - 执行层: tool validator + live suppression
  - 证据: status/digest artifact + suppressed delivery 元信息
  - 失败语义: 实时投递被抑制, durable inbox 仍保留
- `reviewPolicy` / `completionMode`
  - 来源: working agreement + task snapshot
  - 执行层: `team_task_approve` / `team_review_*` validator
  - 证据: review records + task approval events
  - 失败语义: 缺证据即拒绝批准
- `definitionOfDone`
  - 来源: working agreement / kickoff template
  - 执行层: tool validator (若已映射) 或显式标注为软规范
  - 证据: patch/harness_run/test_plan/review 引用
  - 失败语义: 若未映射到执行层, 必须明确写成 "仅口径, 不保证强制执行"
- `helpEscalation`
  - 来源: working agreement / playbook
  - 执行层: help_request artifact 结构 + 升级工具
  - 证据: `help_request` artifact + `team_ask_lead` / `org_principal_message`
  - 失败语义: 超预算或无证据时拒绝升级
...
- mode 切换必须纳入时滞治理:
  - `constrained` / `incident` / `frozen` 不是展示态, 必须真实改变控制输入集合.
  - 解除限制模式必须有冷却窗口与恢复证据, 防止在阈值附近频繁进出造成组织振荡.

#### 15.7.1 Delay Budget 分解 (Pinned)

建议把总时滞拆成可观测的子预算:

- `decisionDelayBudget`
  - 从 ask 创建到 principal/Human sponsor 给出裁决
- `reviewDelayBudget`
  - 从 review request 到结论
- `validationDelayBudget`
  - 从测试/验证开始到证据稳定输出
- `rolloutDelayBudget`
  - 从 release 开始到完成 soak

固定规则:

- 任一子预算耗尽都不应被其他预算掩盖
- change class 越高, 允许的总时滞可以更长, 但必须更显式

#### 15.7.2 长任务 Lease 与心跳约束 (Pinned)

长任务必须避免“沉默运行直到所有人忘记它”:
...
- 推荐字段:
  ...
  - `lastHarnessRunAt`
- 默认规则:
  - 超过 `leaseUntil` 且无新证据时, 任务进入显式关注状态
  - 不能因为执行时间长就默认视作“仍在推进”

#### 15.7.3 多层超时、停滞检测与熔断语义 (Pinned)

Agent Org 的可靠性治理默认不接受“无界等待”:
...
- `streamIdleTimeout`
  - 流式输出长时间无活动时的停滞检测
- `queueAcquireTimeout`
  - 等待 reviewer / runner / merge queue / scarce actuator 的上限

固定规则:

- 任一超时触发后, 系统必须显式进入以下之一:
  - 失败
  - 降级
  - `handoff_pending`
  - `help_request`
  禁止静默继续等待.
- 同一依赖或同一阶段连续超时达到阈值后, 必须触发熔断 / 暂停 / 范围收缩, 防止 7x24 下形成无效重试风暴.
- 超时事件必须可观测并可引用:
  - `stage`
  - `timeoutType`
  - `budgetExceededByMs`
  - `upstreamDependency` (可选)
  - `recoveryAction`
- 对 review queue、CI queue、merge queue、外部 gate 的超时, 默认视为组织级背压信号, 而不只是单任务偶发失败.
...
#### 15.8.1 Governance Artifact Retention Class (Pinned)

- `governance`
  - 包含: constitution、template、handbook、charter、working agreement、ADR/RFC、runbook
  - 规则: 默认不可 hard delete; 即使 content 归档, 也必须保留稳定 `storageRef` 与可恢复读取路径
- `work`
  - 包含: 普通 patch、临时 digest、任务执行证据等
  - 规则: 可按保留窗口归档, 但必须保留最小 tombstone

固定要求:

- 被 pin 的治理产物必须受更严格保留策略保护, 直到被显式 supersede/deprecate.
- 所有 `*Ref` 在回放或读取时必须要么成功解析, 要么返回携带 tombstone 元信息的显式错误; 禁止无提示失效.
...
#### 15.9.1 Exhaustion Degrade Matrix (Pinned)

资源耗尽时, 系统不能“一刀切全停”, 需要按能力降级:

- 允许继续:
  - read/list/pop/ack
  - 显式收口动作: approve/cancel/remove/reassign
  - digest/help_request 发布
- 默认禁止继续:
  - recruit / spawn
  - 新的高成本 review request
  - 非紧急 release
- 条件允许时保留:
  - 低成本 team-local task create
  - 仅引用型 artifact publish

固定规则:

- 降级矩阵必须与 `operatingMode` 联动
- `frozen` 或严重 exhaustion 下, 系统优先保留收口能力而不是扩张能力

#### 15.9.2 Recovery Path from Exhaustion (Pinned)
...
1. 再清理或收口存量:
   - 关闭陈旧 review
   - demobilize 临时成员
   - 合并 digest / help_request
1. 若仍无法恢复:
   - 升级到 President 或对应 Council 做范围收缩/预算调整决策

固定规则:

- 恢复优先级是:
  - 保证读路径与控制面可用
  - 恢复关键 gate 与 incident 处理能力
  - 最后才恢复扩张与并行能力
...
1. 委托边界漂移: 决策: 用户/Human sponsor 位于 org 外部. constitution 变更必须通过显式 artifact 引用与事件更新, 不允许 President 把自己的推断伪装成 sponsor 裁决.
...
1. 控制器冲突: 决策: 同一控制旋钮只允许一个 primary controller 写入 (见 14.19.2). 其他角色只能提案/升级, override 必须带时间窗、原因与恢复条件.
...
1. 运行模式绕行: 决策: `forming/constrained/incident/frozen/archived` 必须改变允许动作集合. 任何工具若无视 mode 继续 `recruit/task_create/release`, 视为实现缺陷而非策略例外.
1. Experience profile 分叉失控: 决策: `Steward` 与 `Flux` 必须共享同一 durable truth 与同一授权模型. 禁止为 `Flux` 复制第二套控制面对象、第二套真相源或第二套事件口径.
1. 事件日志增长: 决策: cursor 使用 `sequence` 保证稳定; 支持可审计的 compaction (见 15.8 的 `*.events.compacted` 口径), 禁止静默删除.
1. 模板漂移与引用失效: 决策: 模板/手册/章程的 pin 版本必须可追溯, 且所有 `*Ref` 解析失败都必须显式报错并携带 tombstone 或 storageRef. 不允许在引用失效时静默回退到复制粘贴或旧缓存.
