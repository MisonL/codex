...
| DOC-001 | Test placement | No Rust tests in core `src/` directories per local constraint | `rg -n "#\\[test\\]|#\\[cfg\\(test\\)\\]" codex-rs/*/src -g "*.rs"` | Existing repo may violate; report facts, do not mutate | Planned | Existing repo has broad source-tree tests: 369 source files, 3055 `#[test]` hits, and 474 `#[cfg(test)]` hits. Follow-up migration and policy gates are split into `docs/plans/2026-05-09-test-placement-policy-migration.md` |
...
| HYGIENE-002 | Follow-up hygiene | DOC-001 planning change is isolated | `git status --short`; `git diff --cached --name-only`; `git diff --check` | Only validation report and migration plan are changed; whitespace check passes | Pass | 2026-05-09 follow-up change is limited to `docs/reviews/CR-CODEX-TUI-CLI-FULL-SURFACE-2026-05-08.md` and `docs/plans/2026-05-09-test-placement-policy-migration.md`; no Rust code changed |
...
| 2026-05-09 | DOC-001 | `rg -l "#\\[test\\]\|#\\[cfg\\(test\\)\\]" codex-rs/*/src -g "*.rs" \| awk -F/ '{count[$2]++} ...'`; matching `#[test]` and `#[cfg(test)]` per-crate scans | 0 | Follow-up split into `docs/plans/2026-05-09-test-placement-policy-migration.md` with recommended hybrid gate and staged crate slices. No Rust code was migrated in this validation loop | terminal output; plan file |
...
Residual non-blocking finding `DEF-001` is now tracked as a separate policy and migration plan in `docs/plans/2026-05-09-test-placement-policy-migration.md`. The quantified scope is 369 source files and 3000+ test markers, so it remains outside this CLI/TUI validation loop and is not a current functional regression from the validated Codex CLI/TUI surface.
